...
namespace Data_Access_Layer
{
    public class DALMission
    {
        ...
        public async Task<Missions> GetMissionDetailByIdAsync(int id)
        {
            return await _cIDbContext.Missions.FindAsync(id);
        }
        ...
        public async Task<string> UpdateMissionAsync(Missions mission)
        {
            try
            {
                var existingMission = await _cIDbContext.Missions.FindAsync(mission.Id);
        ...
        public async Task<string> DeleteMissionAsync(int id)
        {
            try
            {
                var existingMission = await _cIDbContext.Missions.Where(x => !x.IsDeleted && x.Id == id).FirstOrDefaultAsync();
                if (existingMission != null)
                {
